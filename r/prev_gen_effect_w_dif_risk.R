...
for(i in 1:generations){
  initial_pop <- initial_pop %>% 
    mutate(gen_multiplier = case_when(fin_situation<1 ~ sample(rnorm(1:nrow(initial_pop), mean = 1, sd = 0.05), nrow(initial_pop), replace = F),
                                      fin_situation == 1 ~ sample(rnorm(1:nrow(initial_pop), mean = 1, sd = 0.01), nrow(initial_pop), replace = F),
                                      TRUE ~sample(rnorm(1:nrow(initial_pop), mean = 1, sd = 0.1), nrow(initial_pop), replace = F))) %>% 
...
ggsave(plot = plot,"output/prev_gen_effect_w_dif_risk.png", width = 16, height = 9, dpi = 150)
