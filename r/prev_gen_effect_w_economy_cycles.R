...
for(i in 1:generations){
  initial_pop <- initial_pop %>% 
    mutate(gen_multiplier = case_when(fin_situation<1 ~ sample(rnorm(1:nrow(initial_pop), mean = 0.999, sd = 0.002), nrow(initial_pop), replace = F),
                                      fin_situation == 1 ~ sample(rnorm(1:nrow(initial_pop), mean = 1, sd = 0.002), nrow(initial_pop), replace = F),
                                      TRUE ~sample(rnorm(1:nrow(initial_pop), mean = 1.001, sd = 0.002), nrow(initial_pop), replace = F)),
           economy_multiplier = case_when(i >= generations/2 ~ 1.0002,
                                          TRUE ~ 0.9998)) %>% 
    mutate(fin_situation = gen_multiplier*fin_situation*economy_multiplier) %>% 
    mutate(fin_situation = case_when(fin_situation < 0.5 ~ 0.5,
                                     TRUE ~ fin_situation))
  #                                    fin_situation > 2 ~ fin_situation*0.98,
  #                                    TRUE ~ fin_situation))
...
ggsave(plot = plot,"output/prev_gen_effect_w_economy.png", width = 16, height = 9, dpi = 150)
