...
initial_pop <- generate_population(size = 1000, mean_income = 10, sd_percent = 0.05)
...
for(i in 1:generations){
  initial_pop <- initial_pop %>% 
    mutate(gen_multiplier = sample(rnorm(1:nrow(initial_pop), mean = 1, sd = 0.02), nrow(initial_pop), replace = F)) %>% 
...
plot <- ggplot(den_data,aes(x = fin_situation, fill = as.factor(iteration), colour = as.factor(iteration)))+
  ...
  # labs(title = paste("pop = 1000, generations = 100"))+
  geom_density(alpha = 0.01)+
  theme_minimal()+
  theme(legend.position = "none")
  

ggsave(plot = plot,"output/normal_dist_effect.png", width = 16, height = 9, dpi = 150)
